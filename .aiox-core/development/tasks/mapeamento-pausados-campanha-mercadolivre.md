...
4. Pegar o **primeiro MLB** da lista do drawer — esse sim é buscável na aba Anúncios (`Buscar por título, código ou SKU`, número puro, sem prefixo) pra achar o `SKU <valor>`. **⚠️ Existem 2 caixas de busca na tela "Gestão de anúncios" — usar sempre `input[placeholder="Buscar por título, código ou SKU"]` (dentro da página), NUNCA `input[placeholder="Buscar seus produtos ou vendas por título, SKU ou #"]` (topo do site, classe `nav-header-sellers-search__input`) — essa segunda só abre um dropdown de sugestão e nunca filtra a lista de verdade. Ver detalhe completo em `mapeamento-skus-ads-catalogo-mercadolivre.md`, Passo A.**
