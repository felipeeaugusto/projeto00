...
**⚠️ REGRA GERAL OBRIGATÓRIA (14/08/2026, reforçada 14/08/2026 — aplicar no PROCESSO INTEIRO, nunca em pontos isolados) — paciência e confirmação de carregamento, nunca tempo fixo curto:** todo passo deste documento (e do `mapeamento-pausados-campanha-mercadolivre.md`) que espera um clique/busca/navegação gerar algum resultado na tela (abrir um menu, carregar uma página, atualizar uma listagem, abrir um drawer) DEVE confirmar que o resultado esperado realmente apareceu antes de prosseguir — nunca confiar cegamente num tempo fixo curto (`waitForTimeout` de 400ms/600ms/1500ms/2500ms/3000ms/4000ms) como se fosse garantia de que a página carregou. Conexão de internet, cache do Chrome ou a página estar "muito cheia" (muitos anúncios, muitos elementos) podem tornar um tempo fixo insuficiente em momentos diferentes, mesmo pro mesmo tipo de ação, e mesmo num ponto do processo que "sempre funcionou antes".

**Isso não é uma regra pra aplicar só onde um bug já apareceu — é pra todo o processo, desde o início.** O erro real que motivou o reforço desta regra (14/08/2026): a correção original só tinha sido aplicada em 2 pontos específicos (o menu "Alterar" e o conteúdo da página de Alterar) — o resto do pipeline (busca por SKU, busca reversa por MLB, filtro "Pausados", abrir "Ver variações", carregamento inicial da campanha) continuou usando tempo fixo, e mais bugs de timing apareceram exatamente nesses pontos não corrigidos (SKU `WL4000-127V`, MLB `#5000383363`: a página de Alterar foi lida com "COMPETINDO" já renderizado mas a seção "Concorrência no Mercado Livre" ainda não — texto inconsistente). Corrigir só onde o bug já foi visto, e deixar o resto "porque nunca deu problema", é o mesmo erro de novo, só adiado.

**Técnica padrão obrigatória:** depois de qualquer ação que deveria mudar o conteúdo da tela, ler o texto da página 2 vezes seguidas (com um intervalo curto, ex: 1s) — se as 2 leituras baterem exatamente igual (e não for uma mensagem de carregamento tipo "A página está carregando"), a página estabilizou e é seguro ler de verdade. Se não bater, repetir a leitura (até um limite razoável, ex: 8 tentativas) antes de desistir de verdade. Isso vale igual pra: busca por SKU/MLB, clique no filtro "Pausados", clique em "Ver variações", carregamento de uma campanha, abertura do menu de 3 pontinhos, e leitura da página "Alterar". Nunca ler conteúdo logo após uma ação sem essa confirmação, em nenhum ponto do processo.

Regra prática complementar: nunca declarar ausência de um elemento (ex: botão, item de menu) baseado só em não ter aparecido dentro de um tempo fixo curto — sempre tentar de novo com mais espera antes de concluir "não existe". Trabalho **seguro e com dado real** vale mais que trabalho rápido — o próprio incidente que gerou a correção de todo o pipeline (dados errados da Sanduicheira, 52 minutos de rodada precisando ser refeitos) veio de pressa, não de complexidade.

---

## Cuidado com scripts PowerShell e nomes/textos acentuados (13/08/2026)

**Nunca chamar `powershell.exe` de dentro da ferramenta Bash** pra rodar scripts que comparam nome de arquivo, nome de aba ou qualquer texto com acentos (ex: "ANÚNCIOS EM POTENCIAL..."). A codificação dos caracteres acentuados se perde na travessia Bash→`powershell.exe`, e comparações de string do tipo `$_.Name -eq 'ANÚNCIOS EM POTENCIAL - KARZEN ELETRO (1) (1).xlsx'` nunca batem — o script falha silenciosamente (não dá erro, só nunca encontra o arquivo/aba certo). **Correção:** sempre usar a ferramenta PowerShell nativa diretamente para esse tipo de script, nunca `Bash(command: "powershell.exe ...")`.

---

## Passo A — Achar o SKU e a listagem completa de MLBs

**⚠️ Sempre confirmar que a aba certa está sendo usada antes de buscar (achado no piloto do reprocessamento completo, 18/08/2026):** quando várias abas do Chrome (Modo Navegador) estão abertas ao mesmo tempo — sobra de investigações anteriores, por exemplo — é possível confundir a aba de Anúncios/Anúncios Patrocinados com uma aba de outro contexto que também vive sob o mesmo domínio (ex: um dashboard de campanha específica, `ads.mercadolivre.com.br/product-ads/admin/campaigns/.../dashboard`, confundido com a aba de Anúncios Patrocinados, `ads.mercadolivre.com.br/product-ads/admin/ads`). Buscar um MLB na aba errada não dá erro nenhum — só retorna um resultado vazio ou irrelevante, silenciosamente. **Nunca escrever a lógica de encontrar a aba certa do zero** — sempre importar `acharAbaAnuncios`/`acharAbaAdsPatrocinados` de `.aiox-core/development/scripts/modo-navegador/achar-abas-mercadolivre.js` (módulo compartilhado, reforçado pelo hook `check-selector-reuse.js` v4 — ver BLOCO 0-AA do `CLAUDE.md`).
...
**Confirmado 16/08/2026:** o pipeline `packages/karzen/.aiox-runtime/pipeline-pausados-campanha-completo.js` sempre seguiu essa regra corretamente — a constante `SELETOR_BUSCA` usa o seletor certo (`input[placeholder="Buscar por título, código ou SKU"]`) desde a primeira versão do arquivo, confirmado no histórico do git. Um bug real e caro em tempo de investigação aconteceu não porque o pipeline estivesse errado, mas porque um agente (@dev) escreveu vários scripts de diagnóstico ad-hoc durante uma investigação ao vivo, reimplementando a busca do zero com um seletor genérico (`input[type="search"], input[placeholder*="Buscar"]`) em vez de importar `SELETOR_BUSCA`/reusar `analisarSku` do pipeline já validado — os scripts contaminados geraram horas de diagnóstico errado. Ver BLOCO nova no `CLAUDE.md` (16/08/2026): todo script novo de automação de browser deve importar/reusar seletores e funções já validados de um pipeline de produção, nunca reimplementar do zero.
...
6. **Rolar a página inteira dos resultados da busca por SKU, sempre (13/08/2026, corrigido 14/08/2026):** igual à regra já documentada pro drawer "Ver variações" do doc de campanhas — os resultados da busca por SKU também são carregados aos poucos, e um SKU pode ter mais de 2 cards (o Felipe confirmou casos hipotéticos de 6, 20+ cards). **Rolar não é "um número fixo de vezes" — é rolar até a altura da página parar de crescer de verdade** (3 medições seguidas sem crescer), confirmado que a listagem é infinite-scroll real (sem paginação por botão). Nunca ler o resultado sem rolar até esgotar primeiro.

7. **Expandir todo "ID Family" antes de ler o resultado (14/08/2026):** alguns SKUs têm variações agrupadas sob um "Product ID" externo do Mercado Livre — aparece como um card cujo número tem **16 dígitos** (não bate no padrão normal de MLB, 7-11 dígitos), **sem linha "SKU \<valor\>" própria**, com **preço em faixa** ("R$ 239,90 a R$ 279" em vez de um preço único) e a mensagem "Você recebe diferentes valores nas suas variações". Esse card é só um agrupamento colapsado — os MLBs reais estão escondidos dentro dele. Pra revelar: clicar no botão com `aria-label="Expandir anúncios"` daquele card (ícone de seta, fica logo abaixo da miniatura do produto). **Atenção:** esse botão mantém o mesmo `aria-label` mesmo depois de já expandido (não muda pra "Recolher") — nunca esperar a contagem de botões diminuir pra saber quando parar; contar quantos existem uma vez no início e clicar em cada um exatamente 1 vez. Depois de expandir, rolar a página de novo (o conteúdo revelado pode precisar de mais rolagem) antes de ler o resultado final. Caso real validado: SKU `PROSB-3000`, 2 grupos "ID Family", 6 MLBs reais escondidos dentro deles (todos Clássico, nenhum Premium) — sem essa expansão, os 6 MLBs ficariam completamente invisíveis, sem gerar nenhum erro.

**⚠️ Duas questões em aberto, descobertas durante a validação do "ID Family" (14/08/2026), ainda sem solução confirmada — não tratar como resolvidas:**
- Pra MLBs revelados dentro de um "ID Family", a checagem via "Alterar" (item 1 do Caminho secundário abaixo) às vezes não encontra o botão/item mesmo com a espera de paciência aplicada — sem o marcador de "ML inativou o anúncio" que explicaria uma ausência legítima. Suspeita não confirmada: o índice posicional do botão de 3 pontinhos pode não corresponder corretamente à ordem dos MLBs depois que grupos "ID Family" são expandidos (a expansão pode inserir/reordenar botões na página de um jeito que a lógica atual de índice não previu).
- Também pra MLBs revelados assim, o casamento de preço (Caminho secundário, item 3) às vezes não bate com nenhum preço próprio do MLB, mesmo com a seção Concorrência confirmada — o preço capturado pode pertencer a um MLB diferente (talvez do card "ID Family" pai, ou de um MLB irmão), não ao MLB que está sendo checado.
...
**⚠️ Ressalva obrigatória (17-18/08/2026) — o badge da listagem pode estar desatualizado:** mesmo quando a listagem já mostra um badge explícito (GANHANDO/PERDENDO/COMPARTILHANDO/RESTRITO PARA GANHAR), esse badge é um valor cacheado pelo Mercado Livre e pode não refletir o estado real no momento da checagem. **Sempre confirmar via "Alterar" (caminho secundário abaixo) antes de aceitar o badge da listagem como definitivo** — não tratar mais o caminho principal como suficiente sozinho. Dois casos reais confirmaram a divergência: MLB `#4315960981`/SKU `FP100-220V` (listagem mostrava "RESTRITO PARA GANHAR", status real via Alterar era **PERDENDO**) e MLB `#6679980126`/SKU `PAF11B-220V` (listagem mostrava "GANHANDO", status real via Alterar era **COMPARTILHANDO**).
...
1. Pra cada MLB candidato daquela condição (Clássico ou Premium) — já se conhece o número do MLB desde o Passo A, não precisa clicar em menu nenhum pra descobri-lo: **construir a URL direta** `https://www.mercadolivre.com.br/syi/core/modify?itemId=MLB{numero}` e abrir numa aba nova em background (`abrirAlterarPorMlb(context, mlb)`, `packages/karzen/.aiox-runtime/pipeline-pausados-campanha-completo.js` — mecanismo validado 17-18/08/2026, substitui a limitação técnica documentada anteriormente aqui em 13/08/2026, que descrevia o item "Alterar" do menu de 3 pontinhos como não-clicável em nova guia). Essa aba nova **é sempre fechada logo depois de ler o resultado** (bloco `finally`) — a aba de Anúncios original nunca precisa ser navegada pra longe, então **não é mais necessário voltar pro estado limpo nem rebuscar o SKU do zero** entre um MLB candidato e o próximo (a limitação e a correção de "rebuscar do zero" descritas na versão anterior deste passo não se aplicam mais). O erro histórico do índice posicional de botão via `ancestor::div[...]` (ver caso documentado abaixo) também deixa de valer pra este passo específico, já que não há mais clique em botão de 3 pontinhos aqui — continua relevante pra correspondência cabeçalho→bloco do Passo A.1 em geral.
...
**Nota (17-18/08/2026):** o problema de navegação da aba (item 1 acima) está resolvido pelo mecanismo de URL direta — não existe mais clique em botão de 3 pontinhos pra abrir "Alterar", então esse erro específico não pode mais acontecer nesse ponto. A lição sobre `ancestor::` retornar em ordem de documento continua válida como princípio geral pra qualquer seletor XPath futuro, mesmo não sendo mais aplicada aqui.

**O que NÃO conta como confirmação de catálogo, mesmo aparecendo sob "Concorrência no Mercado Livre":** mensagens como "Inativa", "Baixe seu preço", "Crie um vídeo", "Inativo sem estoque" — só **Ganhando/Perdendo/Compartilhando** (ou preço batido, ver regra acima) confirmam catálogo. Esse foi um erro real cometido em 12/08/2026: extração automática lia "Ativa"/"Inativa" como critério, gerando resultado errado pra quase todos os SKUs de um lote inteiro — corrigido depois de conferência com o Felipe.

**⚠️ Correção (17-18/08/2026) — "PREÇO ALTO" NÃO entra mais nessa lista de exclusão — é PERDENDO real:** até 16/08/2026 este documento tratava "PREÇO ALTO" como uma mensagem genérica de aviso (tipo "Baixe seu preço"), a ser ignorada. Isso estava errado. Quando "PREÇO ALTO" aparece, significa que o nosso preço é mais alto que o do concorrente na mesma condição — ou seja, é um resultado real de disputa, equivalente a **PERDENDO**, e deve ser registrado como tal. Caso real: MLB `#6739045854`/SKU `BAS1295P-127V`, confirmado ao vivo expandindo a seção Concorrência (concorrente R$134 vs nosso R$190,83).

**Os 3 formatos da seção "Concorrência no Mercado Livre" (documentado pela primeira vez em 17-18/08/2026 — antes só existiam como comentário no código):**
1. **Numerado "Opção N"** — várias opções listadas, cada uma com sua condição (Clássico/Premium), preço e resultado (GANHANDO/PERDENDO) explícitos.
2. **Sem rótulo** — uma única opção, sem numeração "Opção N", mostrando condição + preço, com o resultado (GANHANDO/PERDENDO) antes ou depois do bloco de preço.
3. **Colapsado** — só aparece um badge (ex: "PREÇO ALTO") + texto genérico, sem tabela de preços visível. É necessário **clicar no cabeçalho da seção** pra expandir e revelar a tabela real por trás do badge. É neste formato que "PREÇO ALTO" aparece, e onde a correção acima se aplica.

**⚠️ Dupla-leitura obrigatória pro formato colapsado (17-18/08/2026):** o formato colapsado teve **100% de correlação** com os 2 erros reais de dado encontrados na madrugada de 17-18/08/2026 (MLB `#4690623743`/SKU `WAF-127V`, entrada fantasma de catálogo; e o caso do PREÇO ALTO acima) — por isso, sempre que um MLB cair no formato colapsado, ler o resultado **duas vezes, de forma independente** (reabrir "Alterar" numa segunda aba nova, reexpandir a seção, reextrair o badge) antes de aceitar o dado. Se a 2ª leitura não bater com a 1ª (ou der erro), **não escolher nenhuma das duas arbitrariamente** — registrar o MLB como status indeterminado e investigar manualmente, nunca aceitar um resultado não confirmado por divergência.

**⚠️ "Restrito para ganhar" também pode aparecer como frase narrativa, não só como badge maiúsculo (achado no piloto do reprocessamento completo, 18/08/2026):** quando o vendedor está impedido de disputar por Experiência de compra ruim, a página "Alterar" (formato colapsado) às vezes mostra a frase **"Você não pode ganhar porque tem experiência de compra ruim."** em vez do badge "RESTRITO PARA GANHAR" em maiúsculas. É a mesma situação, só que descrita em texto corrido nesta tela específica — deve ser tratada exatamente como "RESTRITO PARA GANHAR" (MLB de catálogo confirmado, disputa impedida por Experiência de compra). Caso real: SKU `CKESSTC-ITA5Q`, MLBs `#4277217107` (Clássico) e `#4277230155` (Premium) — os 2 MLBs de catálogo genuínos já citados na regra de "Restrito para ganhar" acima, confirmados de novo aqui com esse formato de texto. Corrigido no código (`extrairBadgeConcorrenciaColapsada`, `pipeline-pausados-campanha-completo.js`).

**⛔ CORREÇÃO CRÍTICA (24/08/2026) — "PREÇO ALTO"/"PREÇO COMPETITIVO" NÃO são catálogo, ao contrário do que a correção de 16-18/08/2026 acima estabeleceu. Esta seção SUBSTITUI o entendimento anterior sobre o formato colapsado.**

**O que mudou:** o badge **"COMPETINDO"**, que aparece perto do título do anúncio (não dentro da seção de Concorrência), é o sinal definitivo de que aquele MLB está genuinamente em disputa de catálogo. **Sem "COMPETINDO" visível, a seção "Concorrência no Mercado Livre" existir NÃO significa catálogo** — mesmo que mostre um badge tipo "PREÇO ALTO" ou "PREÇO COMPETITIVO". Essas 2 mensagens (e qualquer variante parecida) passam a entrar na lista de "não conta como confirmação de catálogo" (junto com "Inativa", "Baixe seu preço", etc. — ver regra de 82 acima).

**Por que a correção de 16-18/08 estava errada:** ela foi baseada só no caso do MLB `#4935565074` (SKU `PROSB-3000`), sem confirmar se aquele badge "PREÇO ALTO" realmente correspondia a uma disputa de catálogo real (com concorrente identificável) ou a uma funcionalidade diferente do Mercado Livre (indicador de competitividade de preço genérico). Reconferindo em 24/08/2026 (Felipe, validação manual + pedido de confirmação a colegas em outros PCs): os MLBs que mostram só "PREÇO ALTO"/"PREÇO COMPETITIVO" (sem "COMPETINDO") **não são de catálogo** — são MLBs "pai". Achado adicional: colegas em outros PCs, abrindo os MESMOS MLBs, não viram a seção "Concorrência no Mercado Livre" aparecer nem para esses casos — reforça que esse badge não reflete uma disputa de catálogo real e estável.

**Os 4 formatos confirmados como catálogo de verdade (todos COM "COMPETINDO" visível, com prints de referência em `C:\Users\Felipe Augusto\Pictures\Processo Oficial\mlb1.png` a `mlb4.png`):**
1. **1 card único** (mlb1.png) — "Você está ganhando! Alcançou o nível máximo de visitas" + badge GANHANDO/PERDENDO/etc. + condição + preço + nível de visitas. (Corresponde ao formato "sem rótulo" já documentado acima.)
2. **Comparação lado a lado** (mlb2.png) — "Anúncio ganhador" vs badge PERDENDO, cada lado com nível de visitas + preço + lista de vantagens (Envios Full, Frete grátis, etc.).
3. **"Opção N" com produto sem estoque** (mlb3.png/mlb4.png) — Opção 1/Opção 2 mostrando "Inativa" em vez de GANHANDO/PERDENDO, com o texto "Todas as opções de venda do seu anúncio estão inativas" — MLB de catálogo confirmado, só que Pausado (já coberto pela regra existente: `statusProduto === 'Pausado'` → `statusCatalogo = 'Inativo'`).
4. **"Restrito para ganhar" narrativo** (achado 24/08/2026 no SKU `CKESSTC-ITA5Q`, MLBs `#4277217107`/`#4277230155` — mesmo caso já documentado desde 18/08/2026 na regra de "Restrito para ganhar" acima, agora reconectado à decisão de catálogo depois de ter caído na trava do "padrão não mapeado" do BLOCO 0-AD) — a frase exata **"Você não pode ganhar porque tem experiência de compra ruim."**, seguida de "Para ganhar, primeiro, melhore sua experiência de compra" / "Revise os problemas..." / "Verificar problemas" / "Experiência de compra RUIM". MLB de catálogo confirmado, `statusCatalogo: "RESTRITO PARA GANHAR"` (ou "Inativo" se Pausado). Diferente do caso PREÇO ALTO/PREÇO COMPETITIVO (que nunca tem "COMPETINDO"), este formato **sempre** tem "COMPETINDO" presente — por isso é seguro reconhecê-lo sem reabrir a brecha revertida em 24/08/2026.

**⚠️ Regra obrigatória, sem exceção — "algo fora do mapeado" (24/08/2026):** se a seção "Concorrência no Mercado Livre" existir, o badge "COMPETINDO" estiver presente, mas o conteúdo não bater com nenhum dos 4 formatos acima — **não classificar como catálogo nem como não-catálogo**. Registrar como indeterminado/pendente e reportar ao Felipe antes de qualquer decisão (ver BLOCO 0-AD, `.claude/CLAUDE.md`). Nunca presumir com base em "parece parecido" — essa foi exatamente a falha que gerou a correção de 16-18/08, que ficou errada por 8 dias sem ninguém perceber.
