...
> **Status:** Draft (v3 — corrige E107, retratado; ver Metadata)
...
Gatilho `/Solucionador` (FR-2) detectado → sequência fixa de portões inicia → cada portão é uma troca de persona visível na mesma conversa (FR-3), exatamente como já validado nesta própria sessão (Orion → Atlas → Finch → Pedro → Morgan → Aria) → reaproveita 4 mecanismos confirmados pela pesquisa (Tier 0, 5-Layer QG Taxonomy, Executor Patterns, Template de Handoff — todos já existem, ver research.json).
...
| Template de Handoff (`handoff-insumos-tmpl.yaml`) | Formato de passagem de insumos entre portões | ✅ Já existe — `squads/squad-creator/templates/handoff-insumos-tmpl.yaml` (E107 retratado 03/09/2026) |
...
Nenhum — o único arquivo novo previsto (Template de Handoff) já existia; E107 foi retratado.
...
- **Iteration 3 (03/09/2026, @dev):** E107 retratado — o Template de Handoff (`handoff-insumos-tmpl.yaml`) sempre existiu; a busca que gerou o E107 (Fase 3) usou `Glob` sem `path` explícito, que buscou a partir do diretório de trabalho do shell (drift pra `packages/karzen`), não da raiz do projeto. O @dev sobrescreveu o arquivo real por engano ao executar a subtask 1.1, percebeu o erro, restaurou o conteúdo original via git, e corrigiu research.json e este spec. Ver retração completa em `.aiox/itens-em-aberto.md`.
