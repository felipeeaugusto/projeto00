...
| **B6** | **Detectar combinação logicamente contraditória em CÓDIGO, não por regra de comportamento.** Caso real: `statusProduto: Ativo` + `statusCatalogo: Inativa/Inativo` é impossível — o @dev viu e apenas reportou, em vez de verificar (violando a memória `feedback_verificar_antes_de_reportar`, que existia e falhou porque depende do agente lembrar). **O padrão de solução já existe e já está validado no próprio pipeline** (`pipeline-pausados-campanha-completo.js` linha 174: *"Nunca aceitar COMPETINDO sem a seção de Concorrência — combinação contraditória"*) — só nunca foi aplicado a este par de campos. Marcando como `erro`, a rede de segurança existente (linha com erro é sempre reprocessada) resolve sozinha. **Veto do Alan: só combinações logicamente impossíveis entram nessa trava, nunca "coisas que parecem estranhas"** — senão vira volume e o trabalho não anda | ❌ **só aqui** |
| **B9** | **Frase de intenção de registro vira gatilho técnico — aprovado pelo Felipe em 29/08.** Caso real (Parte 2): o agente escreveu *"Registro isso como pendência menor, não vou investigar agora pra não desviar"* (sobre o MLB `6667309696` do PAS23-BIV com `statusCatalogo: null`) e **nunca registrou** — não está no `itens-em-aberto.md`, nem no caderno, nem em commit. Pior que esquecer: o Felipe saiu daquela conversa acreditando que a pendência existia. **Solução:** se um agente escreve frase de intenção de registro ("registro isso", "fica pendente", "anoto isso" e variações) **sem editar o `itens-em-aberto.md` na mesma resposta**, o turno é bloqueado — **mesmo mecanismo do `check-handoff-audit.js`, que já existe e já foi testado** *(veto do Pedro + Atlas)*. **Complemento do Alan:** heurística de varredura precisa de critério lógico declarado ANTES de rodar — a heurística daquele momento (`Ativo` + `sem estoque`) deu 100% de falso positivo em 13 verificações ao vivo, porque não tinha relação causal com o bug procurado. Sem critério lógico não é varredura, é pescaria | ❌ **só aqui** |
| **B8** | **Pré-condição do Felipe tem que ser verificada NO MOMENTO da fase, nunca só anunciada antes.** Caso real (Parte 1): eu avisei *"a Planilha só precisa estar fechada na fase 13"* — horas antes. Quando a fase 13 chegou, rodei sem avisar; o script detectou a Planilha aberta e pulou a escrita corretamente, mas o aviso saiu como **uma linha comum no meio do log** e passou batido. Resultado: quase 24h com a Planilha desatualizada, descoberto só porque o Felipe perguntou. **Solução em 2 partes, nenhuma é regra de comportamento:** (1) o script checa se a Planilha está aberta **ANTES de começar** e para pedindo pra fechar — hoje ele roda minutos de automação e só descobre no fim que não pode escrever *(veto do Pedro)*; (2) o aviso de "escrita não realizada" sai do log corrido e vira **bloco destacado no final**, impossível de perder no meio do texto *(veto do Alan + Atlas)* | ❌ **só aqui** |
| **B7** | **Regra nova (decisão do Felipe, 29/08): toda proposta de solução passa a ser feita pelos 4 — Pedro Valério, Alan Nicolas, Thiago Finch e Atlas (`*elicit`) — nunca só pelo Atlas.** Formato obrigatório: cada um escreve sua visão com seu veto explícito, seguido de "Recomendação dos 4" (o que sobrevive aos 4 vetos) e do que ficou não resolvido. Motivo: comprovado na prática nesta sessão — a análise de 1 agente para no primeiro desenho coerente; a dos 4 continua até o desenho quebrar. Foi assim que apareceram o B6, a A13, a A14 e a A15, todos invisíveis na análise solo | ❌ **só aqui** |
