...
- [14/08/2026] analyst (via *elicit) — "ID Family" (agrupamento colapsado dentro de um card que precisa clicar numa setinha pra revelar MLBs escondidos) nunca apareceu em nenhum SKU processado até agora (Taiff, WW2, WL4000) e não tem NENHUMA implementação no pipeline -- se algum SKU já "validado" tinha essa estrutura sem a gente perceber, pode ter MLBs perdidos silenciosamente. Precisa ser projetado e implementado do zero -- arquivo: `packages/karzen/.aiox-runtime/pipeline-pausados-campanha-completo.js`
