...
(async () => {
  ...
  try {
    ...
    await itemMesclar.focus();
    await page.waitForTimeout(300);
    await page.keyboard.press('ArrowRight');
    await page.waitForTimeout(600);

    const itensAgora = await page.locator('.goog-menuitem').allTextContents();
    console.log('Itens apos ArrowRight:', JSON.stringify(itensAgora.filter(t => /mescl/i.test(t))));
    ...
    if (await itemDesfazer.count() > 0) {
      ...
      if (disabled !== 'true') {
        await itemDesfazer.click({ timeout: 5000, force: true });
        await page.waitForTimeout(500);
