...
(async () => {
  ...
  try {
    ...
    // Procurar botoes/elementos com "mesclar" ou "merge" no aria-label ou title
    const candidatos = await page.locator('[aria-label*="esclar" i], [aria-label*="merge" i]').all();
    console.log(`Elementos com "mesclar/merge" no aria-label: ${candidatos.length}`);
    for (const el of candidatos) {
      const label = await el.getAttribute('aria-label').catch(() => null);
      const id = await el.getAttribute('id').catch(() => null);
      console.log(`  aria-label="${label}" id="${id}"`);
    }
