...
(async () => {
  ...
  try {
    ...
    // Reabrir dropdown pra confirmar qual opcao esta marcada agora
    await irParaCelula(page, 'C7');
    await page.waitForTimeout(300);
    await wrapBtn.click({ timeout: 5000 });
    await page.waitForTimeout(500);
    const opcoes = await page.locator('.goog-menu:visible [aria-label]').all();
    for (const el of opcoes) {
      ...
      console.log(`  ${label}: aria-pressed=${pressed} aria-selected=${selected}`);
    }
